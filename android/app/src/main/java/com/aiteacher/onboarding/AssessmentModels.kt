...
data class SessionItem(
    ...
    val subject: String = "",
    val topicRank: Int = 0,          // 1 = highest priority
    val isRevision: Boolean = false,
    val weaknessScore: Int = 0       // 0-100, higher = weaker
)
...
/**
 * A single topic from the curriculum, ranked and annotated by TopicRankerAgent.
 */
data class RankedTopic(
    val id: String,                        // e.g. "math_quadratic_equations"
    val subject: String,                   // e.g. "math"
    val title: String,                     // e.g. "Quadratic Equations"
    val rank: Int,                         // 1 = teach first
    val importanceScore: Int,              // 0-100: exam weight
    val dependencyScore: Int,              // 0-100: how many topics depend on this
    val currentTermScore: Int,             // 0-100: how likely being taught right now
    val weaknessScore: Int,                // 0-100: from test results
    val dependsOn: List<String> = emptyList(), // ids of prerequisite topics
    val isCurrentTerm: Boolean = false,
    val suggestedSessionCount: Int = 1     // how many sessions this topic needs
)
