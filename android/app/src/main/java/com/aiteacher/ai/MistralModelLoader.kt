...
class MistralModelLoader(private val baseUrl: String = "https://api.mistral.ai") : ModelLoader {
    private val client = OkHttpClient()
    private val gson = Gson()

    override suspend fun listModels(apiKey: String): List<ModelInfo> {
        try {
            // Mistral may expose /v1/models or /models
            ...
            if (json.has("models") && json.get("models").isJsonArray) {
                ...
                arr.forEach { el ->
                    if (el.isJsonObject && el.asJsonObject.has("id")) out.add(ModelInfo(el.asJsonObject.get("id").asString))
                }
            }
            if (out.isEmpty() && json.has("data") && json.get("data").isJsonArray) {
                val arr = json.getAsJsonArray("data")
                arr.forEach { el -> if (el.isJsonObject && el.asJsonObject.has("id")) out.add(ModelInfo(el.asJsonObject.get("id").asString)) }
            }
            return out
        } catch (e: Exception) {
            Log.w("MistralModelLoader", "error listing models: ${e.message}")
            return emptyList()
        }
    }
}
