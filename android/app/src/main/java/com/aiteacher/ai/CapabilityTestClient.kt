...
object CapabilityTestClient {
    ...
    suspend fun generateTest(
    ...
    ): CapabilityTest = withContext(Dispatchers.IO) {
        ...
        if (!apiKey.isNullOrBlank() && !provider.isNullOrBlank()) {
            try {
                ...
                if (parsed.size >= 5) return@withContext CapabilityTest(parsed)

                Log.w("CapabilityTestClient", "First attempt got ${parsed.size} questions, retrying…")
                val retryRaw = callAi(context, provider, apiKey, model, buildSimplePrompt(profile, subjects, questionCount))
                val retryParsed = parseQuestionsFromJson(retryRaw)
                if (retryParsed.isNotEmpty()) return@withContext CapabilityTest(retryParsed)
            } catch (e: Exception) {
        ...
        CapabilityTest(builtInAdaptiveFallback(profile, subjects))
    }
